...
async function BlogList() {
  ...
  let list = [];
  try {
    const result = await getBlogList(1, 10);
    list = result?.list || [];
    console.log('博客列表:', list);
  } catch (error) {
        ...
        <div className={styles.blogGrid}>
          {list?.map((blog) => (
            <article className={styles.blogCard} key={blog.id}>
              <Link href={`/blog/${blog.id}`} className={styles.cardLink} prefetch={false}>
                <div className={styles.cardHeader}>
                  <span className={styles.category}>{blog.category}</span>
                  {(blog.readTime || blog.read_time) && (
                    <span className={styles.readTime}>{blog.readTime || blog.read_time} 分钟阅读</span>
                  )}
                </div>
                ...
                <div className={styles.cardFooter}>
                  ...
                  <div className={styles.meta}>
                    ...
                    <span className={styles.date}>{handleTimeBlog(blog.created_at || blog.date)}</span>
                  </div>
                </div>
              </Link>
            </article>
